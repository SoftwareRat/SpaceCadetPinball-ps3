...
void options::InitPrimary()
{
    ...
    // PS3/SDL2 Default Mapping (3 Bindings pro Aktion wie im PC-Original)
	Options.Key = Options.KeyDft =
    {
        { // Left Flipper -> L1 (oder alternativ Steuerkreuz Links)
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_LEFTSHOULDER},
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_DPAD_LEFT},
            {InputTypes::None, 0},
        },
        { // Right Flipper -> R1 (oder alternativ Steuerkreuz Rechts)
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER},
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_DPAD_RIGHT},
            {InputTypes::None, 0},
        },
        { // Plunger -> Kreuz / X-Button (oder Analog-Stick runter)
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_A}, // SDL_CONTROLLER_BUTTON_A entspricht 'X' auf PlayStation
            {InputTypes::None, 0},
            {InputTypes::None, 0},
        },
        { // Left Table Bump -> L2
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_BACK}, // Select für Ruckeln
            {InputTypes::None, 0},
            {InputTypes::None, 0},
        },
        { // Right Table Bump -> R2
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_START}, // Start für Menü / Pause
            {InputTypes::None, 0},
            {InputTypes::None, 0},
        },
        { // Bottom Table Bump -> Steuerkreuz Unten
            {InputTypes::Keyboard, SDL_CONTROLLER_BUTTON_DPAD_DOWN},
            {InputTypes::None, 0},
            {InputTypes::None, 0},
        }
    };
...
void options::GetInput(const std::string& rowName, GameInput (&defaultValues)[3])
{
    for (auto i = 0u; i <= 2; i++)
    {
        ...
        if (inputType <= InputTypes::Mouse && input != -1) // PS3 Fix: Limit auf InputTypes::Mouse
